...
//             let balance_factor = self.get_balance_factor(node.clone());
//             let key = self.get_key(node.clone());
//             // println!("For node {} Balance factor: {}", n.key, balance_factor);
//             println!("For node {} Balance factor: {}", key.clone(), balance_factor);
...
//             if let Some(p) = self.get_parent(current_node.clone()) {
...
//         node.as_ref().borrow().get_balance_factor()
//     }
